...
function Sidenav({ color }) {
  ...
  const callEffecFunc = async()=>{
    ...
    console.log(data);
    if(response.status === 200 && data.RoleId === 1){
    setState(true);
    }else if(response.status === 200 && data.RoleId === 2){
      setState(false);
    }
  }
    ...
    <>
      ...
      <Menu theme="light" mode="inline">
       ...
       <Routing />
          ...
          <span className="icon">{signup}</span>
          <span className="label" onClick={logOutFunc}>Logout1</span>
